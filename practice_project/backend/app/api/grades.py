...
async def get_grade(grade_id: int, db: AsyncSession = Depends(get_db), role: str = Depends(get_current_role)):
    if not check_permission(role, "read"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Недостаточно прав")
...
async def create_grade(grade_data: GradeCreate, db: AsyncSession = Depends(get_db), role: str = Depends(get_current_role)):
...
async def update_grade(grade_id: int, grade_data: GradeUpdate, db: AsyncSession = Depends(get_db), role: str = Depends(get_current_role)):
...
async def delete_grade(grade_id: int, db: AsyncSession = Depends(get_db), role: str = Depends(get_current_role)):
